...
{library(sf); ua <- read_sf(dsn = fp, layer = file); union_vector <- character()
  ...
  nn_df <- data.frame(nn_UACE20 = character(), dist = double(),
                      UACE20 = character())}

#loop code block (runtime inverse proportional to cdist, ~19h on laptop)
for (iter in 1:nrow(ua)) {if (ua$UACE20[1] %in% dist_df[, 3]) {break}
ua$dist <- as.vector(st_distance(ua[ua$UACE20 == ua$UACE20[1], ], ua))
...
nn <- nn[which.min(nn$dist), ]; names(nn) <- c("nn_UACE20","dist","UACE20")
distrows <- distrows2 - distrows1
if (distrows > 1) {ids2check <- setdiff(dist_df$UACE20[1:distrows],
                                        ua$UACE20[1])
union_vector <- c(ua$UACE20[1], ids2check[1])
...
while(length(ids2check) > 0) {
  ...
  nn2 <- nn2[which(!(nn2$UACE20 %in% union_vector)), ]
  nn2 <- nn2[which.min(nn2$dist), ]
  names(nn2) <- c("nn_UACE20", "dist", "UACE20"); nn <- rbind(nn, nn2)
  nn <- nn[which(!(nn$nn_UACE20 %in% union_vector)), ]
  ...
  union_vector <- union_vector[!is.na(union_vector)]}
u <- st_sf(st_union(ua[ua$UACE20 %in% union_vector, ]))
nn <- nn[which(!(nn$nn_UACE20 %in% union_vector)), ]
...
#clean up code block
{dist_df <- dist_df[which(dist_df$UACE20 != dist_df$id2), ]
...
agglom_meta[[9]] <- agglom_meta[[3]]/agglom_meta[[6]]
